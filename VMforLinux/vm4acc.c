...
static int proc_num;
unsigned int vclock;
extern unsigned int cycles[256], ext_cycles[256], stats[256], ext_stats[256];

int high_mem = 0xfefe; // end of transient memory
byte mem[65536]; // additionnal space due to bug in EDIT2
byte *MEM(int addr)  {
#if PAGE0_CHECK
...
void   push(word w)   { STACK_PTR-=2; STACK[0] = TOP; TOP = w; }
void  dpush(dword dw) { STACK_PTR-=4; STACK[1] = TOP; *(dword *)(STACK-1) = dw; TOP = STACK[-1]; }
void  fpush(float f)  { STACK_PTR-=4; STACK[1] = TOP; *(float *)(STACK-1) =  f; TOP = STACK[-1]; }

word   pop(void)      { word res = TOP; TOP = STACK[0]; STACK_PTR+=2; return res; }
sword ipop(void)      { return pop(); }
word  ppop(void)      { if (TOP==0) error(POINTER_ERROR,0,0); return pop(); }
dword dpop(void)      { STACK[-1] = TOP; dword d = *(dword *)(STACK-1); TOP = STACK[1]; STACK_PTR+=4; return d; }
float fpop(void)      { STACK[-1] = TOP; float f = *(float *)(STACK-1); TOP = STACK[1]; STACK_PTR+=4; return f; }
...
void  swap() { word tmp=STACK[0]; STACK[0] = TOP; TOP = tmp; }
...
{
    STACK_PTR         = LOCAL_PTR;
    RESERVE_STACK_PTR = STACK[-1];
    OUTER_FRAME       = STACK[ 0];
    LOCAL_PTR         = STACK[ 1];
    IP                = STACK[ 2];
    STACK_PTR        += 6;     // drop these 3 pointers
...
void leave(int n)
{
    ...
    leave_check(n);
    // in this version, we reload the TOP register
    TOP = STACK[0]; STACK_PTR += 2;
}
...
{
    // in this version, TOP contains the result
    restore_frame(2*n & 0xff);
    leave_check(n);
}
...
{
    word lsw = pop(); // and so now TOP contains the msw
    restore_frame(2*n & 0xff);
    push(lsw);
    leave_check(n);
}
...
void copy_string()
{
    ...
    vclock += src_size * 15 + (dst_size - src_size) * 7; // length(src) would be better
...
void enter(int n)
{
    word size = 0xff - n;
    push(RESERVE_STACK_PTR);
    LOCAL_PTR = STACK_PTR;     // in this version, the last push is actually not on stack
    push(0);                   // force the previous push, and load TOP with 0
...
void extend_opcode(void) {
    ...
    unsigned int vclock0 = vclock;
    vclock += ext_cycles[opcode];

    switch (opcode) {
    case  0: (void)pop(); break; // drop
    ...
    case  4: op2=pop(); op=pop(); push( (1<<(op2+1))-(1<<op) ); vclock += (op+op2)*5; break;
...
// new opcodes
    ...
    case 28: op2 = pop(); op = pop(); push(bios(op,op2)); 
             if (op==4 || op==5) vclock += 20000; // approx. for sector read/write
             break; // SYS
    case 29: iop2=ipop(); iop=ipop(); push(iop>=0 ? iop%iop2 : (iop%iop2 + iop2) % iop2); break; // signed Modulo
    case 30: op2=FETCH; dop=(dword)ipop(); push((sword)(dop >> op2)); vclock += op2*5; break; // ASR
    ...
    case 35: op = pop();  push( ipop() >> op ); vclock += op*5; break;
    case 36: op = pop(); dpush( dpop() >> op ); vclock += op*7; break;
    case 37: op = pop();  push( ipop() << op ); vclock += op*5; break;
    case 38: op = pop(); dpush( dpop() << op ); vclock += op*7; break;
    default:
        unimplemented(*MEM(--IP));
    }
    ext_stats[opcode] += vclock - vclock0;
}
...
//    fprintf(tracelog," [FEEE]=%04x",*WMEM(0xFEEE));
  ...
  case 0x12: unimplemented(0x12); break;
  ...
  case 0x87: unimplemented(0x87); break; // NATIVE code
  ...
  case 0xa2: LOCAL[IFETCH]++; break; // replaces old opcode LOWER_THAN
  case 0xa3: LOCAL[IFETCH]--; break; // replaces old opcode HIGHER_THAN
  case 0xa4: // replaces old opcode LOWER_OR_EQ
             iop=ipop(); itmp=LOCAL[IFETCH]; tmp = WFETCH;
             if (itmp <= iop) { push(iop); IP+=tmp-1; }
             break; 
  case 0xa5: // replaces old opcode HIGHER_OR_EQ
             iop=ipop(); itmp=LOCAL[IFETCH]; tmp = WFETCH;
             if (itmp >= iop) { push(iop); IP+=tmp-1; }
             break; 
  ...
  case 0xb0: op=FETCH;   push( pop() << op ); vclock += op * 5; break;
  case 0xb1: op=FETCH;   push( pop() >> op ); vclock += op * 5; break;
  ...
  case 0xea: op=pop(); push( 1 << op ); vclock += op * 5; break;
